...
;; Part 1
(define remove-last-negative
  (lambda (lst)
    (define neg (getneg lst))
    (reverse lst)
    (if(null? lst)
       '()
       (if(equal? (car lst) neg)
          (cdr lst)
          (cons (car lst) (remove-help (cdr lst) neg))))))
...
(define remove-help
  (lambda (lst n)
    (if(null? lst)
       ...
       (if(equal? (car lst) n)
          (cdr lst)
          (cons (car lst) (remove-help (cdr lst) n))))))
...
;; Part 4
(define squares-up-to
  (lambda (x)
    (if (< x 1)
        '()
        (if (integer? (sqrt x))
          (squares-helper  (cons x '()) (- x 1))
               (squares-helper '() (- x 1))))))

 (define squares-helper
   (lambda (lst x)
     (if (< x 1)
         lst
         (if (integer? (sqrt x))
          (squares-helper  (cons x lst) (- x 1))
               (squares-helper lst (- x 1))))))

;; Part 5
(define election-results
  (lambda (lst)
    (define res (results-helper lst (cons 0 0)))
    (if (< (car res) (cdr res))
         "B"
         "A")))

 (define results-helper
   (lambda (lst res)
     (if (null? lst)
     res
     (if(< (car (cdr (car lst))) (cdr (cdr (car lst))))
        (results-helper (cdr lst) (cons (car res) (+(car (car lst)) (cdr res))))
        (results-helper (cdr lst) (cons  (+(car (car lst)) (car res)) (car res)))))))
